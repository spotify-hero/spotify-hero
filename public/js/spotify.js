...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("button-pause").onclick = () => {
    spAPI.pause(access_token);
  };
  document.getElementById("button-play").onclick = () => {
    spAPI.resume(access_token);
  };
  document.getElementById("button-previous").onclick = () => {
    spAPI.previous(access_token);
  };
  document.getElementById("button-next").onclick = () => {
    spAPI.next(access_token);
  };
  document.getElementById("keywords").onchange = () => {
    spAPI.search("keywords", access_token);
  };
  document.getElementById("keywords_button").onclick = () => {
    spAPI.search("keywords", access_token);
  };
  document.getElementById("trackID").onchange = () => {
    spAPI.searchOne("trackID", access_token);
  };
  document.getElementById("trackID_button").onclick = () => {
    spAPI.searchOne("trackID", access_token);
  };

  document.getElementById("button-database").innerHTML =
    '<a href="/select?table=Track+Score&access_token=' +
    access_token +
    '">Part 2 : Database</a>';
  ...
  var cpTemplate = Handlebars.compile(
      document.getElementById("currently-playing-template").innerHTML
    ),
